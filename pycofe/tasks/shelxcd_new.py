    ...
    def run(self):
        ...
        for i in range(len(hkl)):
            ...
            if hkl[i].cols[4] != "X":
                ...
                if hkl[i].cols[4] == "F":
                    hkl[i].cnvfname = (
                        os.path.splitext(hkl[i].getHKLFileName())[0] + ".hkl"
                    )
                    self.open_stdin()
                    self.write_stdin(
                        [
                    ...
                    hkl[i].cnvfname = (
                        os.path.splitext(hkl[i].getHKLFileName())[0] + ".sca"
                    )
                    self.runApp(
                        "mtz2sca",
                        [
        ...
        if hasattr(self.input_data.data, "native"):  # optional data parameter
            ...
            if native.cols[2] != "X":
                ...
                self.runApp(
                    ...
                    [
                        "HKLIN",
                        native.getHKLFilePath(self.inputDir()),
                        "HKLOUT",
                        native.cnvfname,
                    ],
                    logType="Service",
        ...
        self.open_stdin()
        self.write_stdin(
            ["CELL " + hkl0.getCellParameters_str(), "SPAG " + hkl0.getSpaceGroup()]
        )
        if expType == "SIRAS":
        ...
        if os.path.isfile(pdbfile):
            ...
            if structure:
                ...
                if hkla:
                    # self.putMessage ( "&nbsp;" )
                    anom_structure = self.finaliseAnomSubstructure(
                        pdbfile, "anom_substructure", hkla, [], "", openState="hidden"
                    )
                    if anom_structure:
                ...
                pyrvapi.rvapi_add_grid(
                    gridId, False, self.report_page_id(), self.rvrow, 0, 1, 1
                )
                self.rvrow += 1
                ...
                for i in range(len(hkl_all)):
                    ...
                    if len(hkl_all) == 1:
                        ...
                        self.putRevisionWidget(
                            gridId, i, "New structure revision name:", ri
                        )
                    else:
                        ri.makeRevDName(
                            self.job_id,
                        ...
                        self.putRevisionWidget(
                            gridId, i, hkl_all[i].wtype + " dataset:", ri
                        )
                ...
                if have_results:
                    self.generic_parser_summary["shelxcd"] = {
                        "summary_line": revision.ASU.ha_type
                        + "<sub>"
