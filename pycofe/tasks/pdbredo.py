...
class Pdbredo(basic.TaskDriver):
    ...
    def formStructure ( self,xyzout,subfile,mtzout,libin,hkl,istruct,maplabels,copyfiles ):
        ...
        if structure:
            ...
            structure.setPdbredoLabels    ( None if str(hkl.useHKLSet) in ["Fpm","TI"] else hkl )
            if not subfile:
    ...
    def run(self):
        ...
        self.setPdbredoLogParser ( self.pdbredo_report(),False )

        # Start
        self.runApp ( "pdbredo",cmd,logType="Main" )
        self.unsetLogParser()
