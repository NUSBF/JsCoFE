        ...
        for chain in model:
            ...
            for residue in chain:
                if residue.entity_type==gemmi.EntityType.Polymer:
                    ...
                    for atom in residue:
                        ligand_atoms    += 1
                        ligand_b_factor += atom.b_iso
                        if atom.element==gemmi.Element('H') or atom.element==gemmi.Element('D'):
                            hydrogen_atoms += 1

    # Compute average B-factors
    avg_b         = 0.0
    overall_atoms = macromolecular_atoms + ligand_atoms + water_atoms
    if overall_atoms>0:
      avg_b = (macro_b_factor + ligand_b_factor + water_b_factor ) / overall_atoms
    avg_macro_b  = macro_b_factor  / macromolecular_atoms  if macromolecular_atoms  else 0
    avg_ligand_b = ligand_b_factor / ligand_atoms          if ligand_atoms          else 0
