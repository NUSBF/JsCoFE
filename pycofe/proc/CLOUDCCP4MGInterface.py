...
def InstallSaveToCloudMenuItem(workDirectory):
...
def SetupSequenceLoadingFromCommandLine():
    ...
    if mrbumpwin_tmpdir is not None:
        try:
            ...
            if hasattr(mrbumpwin,"raise_"):
                 QtCore.QCoreApplication.instance().connect(QtCore.QCoreApplication.instance(),QtCore.SIGNAL("ApplicationReady"),mrbumpwin.raise_)
            return tmpdir
        except:
            pass
    return None
...
def InstallSaveEnsembleToCloudMenuItem(workDir):
    ...
    menu_defn = {}
    menu_defn['text'] = "Save all visible to CCP4 Cloud"
