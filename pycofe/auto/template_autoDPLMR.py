...
def makeNextTask ( crTask,data ):
    ...
    if crTask._type=="TaskWFlowDPLMR":
        ...
        if len(data["unm"]) > 0:
            ...
            auto_tasks.dimplemr("dimplemr", crTask.autoRunName)   

        # if len(data["seq"])>0:
        #     auto_api.addContext("seq", data["seq"])
        #     auto_tasks.editrevision ("TaskEditRevision", data["revision"], crTask.autoRunName )
        # else:
        #     auto_api.getContext("data")
        #     auto_tasks.dimplemr("dimplemr", data["data"], crTask.autoRunName)
        return
    ...
    elif crTask._type=="TaskDimpleMR":
