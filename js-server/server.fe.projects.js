...
/*  --- node-based zip version (inefficient) 26.07.2018
function prepareProjectExport ( login,projectList )  {

  log.standard ( 9,'export project "' + projectList.current + '", login ' + login );

  var projectDirPath = getProjectDirPath ( login,projectList.current );
  var tarballPath1   = path.join ( projectDirPath,projectList.current + '.zip' );
  utils.removeFile ( tarballPath1 );  // just in case

  var tarballPath    = path.join ( projectDirPath,'__' + projectList.current + '.zip' );

  var output  = fs.createWriteStream ( tarballPath );
  ...
  archive.directory ( projectDirPath,false );

  // 'close' event is fired only when a file descriptor is involved
  output.on ( 'close',function(){
    if (errors)  {
      log.error ( 10,errors + ' at packing project ' + projectList.current );
      utils.removeFile ( tarballPath );
    } else  {
      log.standard ( 10,'project ' + projectList.current + ' packed in ' + archive.pointer() + ' bytes' );
      utils.moveFile   ( tarballPath,tarballPath1 );
    }
  });
...
function checkProjectExport ( login,projectList )  {
  var projectDirPath = getProjectDirPath ( login,projectList.current );
  var tarballPath    = path.join ( projectDirPath,projectList.current+'.zip' );
  rdata = {};
  if (utils.fileExists(tarballPath))
        rdata.size = utils.fileSize(tarballPath);
  else  rdata.size = -1;
...
function finishProjectExport ( login,projectList )  {
  var projectDirPath = getProjectDirPath ( login,projectList.current );
  var tarballPath1   = path.join ( projectDirPath,projectList.current+'.zip' );
  var tarballPath2   = path.join ( projectDirPath,'__' + projectList.current+'.zip' );
  utils.removeFile ( tarballPath1 );
  utils.removeFile ( tarballPath2 );
  return new cmd.Response ( cmd.fe_retcode.ok,'','' );
}
