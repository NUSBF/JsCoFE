...
function ProjectPage ( sceneId )  {
  ...
  */
  ...
  function onTreeContextMenu(_node) {
  ...
  //     jobTree.readProjectData ( 'Project',false,-1,function(){
  //       jobTree.multiple = job_tree.multiple;
  ...
  function reloadTree ( blink,rdata )  {
    ...
    if (jobTree && jobTree.parent)  {
      // var selTasks  = jobTree.getSelectedTasks();
      jobTree.stopTaskLoop();
      jobTree.checkTimeout = -1;  // prevents task loop from starting again
      var scrollPos = jobTree.parent.getScrollPosition();
      var jobTree1  = self.makeJobTree();
      var timestamp = jobTree.projectData.desc.timestamp;
      if (blink)  {
        ...
        jobTree1.hide();
      jobTree1.readProjectData ( 'Project',false,timestamp,function(){
        var tree_parent         = jobTree.parent;
        var dlg_task_parameters = jobTree.getJobDialogTaskParameters();
        if (jobTree1.projectData)  {
        ...
        jobTree.checkTimeout = null;  // allows task loop to start
        jobTree.startTaskLoop();
        // } else  {
        //   // revert back to the previous Tree
        //   self.job_tree    = job_tree;
